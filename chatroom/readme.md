...
| | Android（2026-09-15 起） | iOS（当前） |
|---|---|---|
| 主页 | **菜单页**：所有会话竖列成一级大卡片（默认折叠，点开是参与者编辑二级卡片），最底下大加号新建；新建先落**草稿卡片**，点「创建」才真正建会话 | 编辑卡片列表 + 顶部「创建」按钮（一次编辑、一次创建） |
| 进聊天 | 底部 tabbar，或菜单卡片里的「进入会话」 | 底部 tabbar |
| 会话管理入口 | 主页菜单页，以及聊天页左上角 ☰ 打开的 **80% 屏宽叠加层**（同一份菜单页盖在原画面上方） | 聊天页 tab 上点 ⓘ 展开重连面板 |

会话持久化 / 重连 / tab 删除线两端一致，见 [`android.md`](./android.md) / [`ios.md`](./ios.md)。
